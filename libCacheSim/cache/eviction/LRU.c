//
//  LRU (Least Recently Used) 缓存替换算法实现
//  支持不同对象大小的LRU模块
//
//  LRU是一种经典的缓存替换策略，它基于这样的假设：最近访问过的对象在不久的将来可能会再次被访问。
//  当缓存满时，LRU会淘汰最长时间未被访问的对象。
//
//  算法特点：
//  - 每次访问对象时，将其移动到缓存队列的头部
//  - 当需要淘汰对象时，选择队列尾部的对象（最长时间未访问的）
//  - 时间复杂度：查找O(1)，更新O(1)，插入O(1)，删除O(1)
//
//  LRU.c
//  libCacheSim
//
