//
//  Belady (MIN) 缓存替换算法实现
//  也称为MIN或最优替换算法(OPT, Optimal Replacement Policy)
//
//  Belady算法特点：
//  - 理论上最优的缓存替换策略
//  - 总是淘汰最长时间内不会被再次访问的对象
//  - 需要预知未来的访问序列，因此在实际系统中无法实现
//  - 常被用作理论上限来评估其他缓存替换算法的性能
//  - 时间复杂度：查找O(1)，淘汰O(log n)
//
//  实现方式：
//  - 使用优先队列(priority queue)记录每个对象的下一次访问时间
//  - 当需要淘汰对象时，选择下一次访问时间最远的对象
//  - 如果对象不会再被访问，则优先淘汰
//
//  Belady.c
//  libCacheSim
